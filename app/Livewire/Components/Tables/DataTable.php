...
use App\Livewire\Traits\HandlesJoinSorting;
use App\Livewire\Traits\HandlesJoinSearching;


class DataTable extends Component
{
    use WithPagination,HandlesJoinSorting,HandlesJoinSearching;
    ...
    public function mount()
    {
        ...
        $this->searchableColumns = collect($this->columns)
            ->reject(fn($col) => Str::endsWith($col, '_list'))
            ->values()
            ->all();
        ...
        $this->sortableColumns = collect($this->columns)
            ->reject(fn($col) => Str::endsWith($col, '_list'))
            ->values()
            ->all(); // no filtering – now all are allowed
    }
    ...
    protected function getModelQuery(): Builder
    {
        ...
        if ($this->enableSearch && !empty($this->search)) {
            $query = $this->applyJoinSearch($query, $this->model, $this->searchColumn, $this->search);
        }


        // Sort
        if ($this->enableSort && $this->sortColumn !== '') {
            $query = $this->applyJoinSort($query, $this->model, $this->sortColumn, $this->sortDirection);
        }
